...
public class GameMessageDecoder extends ByteToMessageDecoder {
    ...
    public GameMessageDecoder(MessageRegistry registry) {
    ...
    private boolean fullMessageArrived(ByteBuf bb) {
        // 看消息头是否完整
        if (bb == null || bb.readableBytes() < GameMessageProtocol.HEADER_LENGTH) {
            return false;
        }
        bb.markReaderIndex();
        // 检查MagicNumber
        ...
        // 看消息是否完整到达
        final int bodyLength = bb.readUnsignedShort();
        //System.out.print("++++++++++" + bodyLength);
        bb.resetReaderIndex();
        return bb.readableBytes() >= GameMessageProtocol.HEADER_LENGTH + bodyLength;
    }
    ...
    private Object decodeMessage(ByteBuf bb) throws InvalidProtocolBufferException {
        ...
        int bodyLength = bb.readUnsignedShort();
